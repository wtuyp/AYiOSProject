...
open class YAxisRenderer: NSObject, AxisRenderer
{
    ...
    @objc public var fillAreaColor: NSUIColor = NSUIColor.clear
    @objc public var fillAreaMaxLimit: CGFloat = 0.0
    @objc public var fillAreaMinLimit: CGFloat = 0.0
    ...
    // 背景区域填充
    open func renderAreaFill(context: CGContext) {
        guard let transformer = self.transformer else { return }
        // 如果颜色是clear，则不作填充处理
        guard fillAreaColor != NSUIColor.clear else { return }
        // 如果没配范围，则不作填充
        guard fillAreaMinLimit != fillAreaMaxLimit else { return }
        
        let lower = fillAreaMinLimit
        let upper = fillAreaMaxLimit
        
        context.saveGState()
        
        let trans = transformer.valueToPixelMatrix
        
        var startPosition = CGPoint(x: 0.0, y: upper)
        startPosition = startPosition.applying(trans)
        
        var endPosition = CGPoint(x: 0.0, y: lower)
